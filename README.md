...
| Production Company    | Tomatometer Status     | Tomatometer Ratings     |
...
Most of the remaining columns were object/string data type and had way too many unique values for a traditional binning process to later encode, so we had to think of other ways to engineer features.
...
The data was split into training and testing sets using the scikit-learn function `train_test_split`. We used the default settings, so our training set was 75% of the data and the testing set was 25%.
...
We decided to revert back to a [model with more simplified input features](dashboard/dashboard_ML_model.ipynb), as the accuracy was comparable at 69%. We connected our trained model directly to our dashboard to make a prediction based on the user's input, and this simplified the input preprocessing in order to have something functional based on time constraints. 
...
### Tools
We primarily used Flask to create our web application. An HTML form allows users to input information that is passed to the Flask server in order to run a Python script to predict results using our saved machine learning model. 

### Interactive Elements
The main interactive element is the Status Prediction Form. After the user enters information about the movie and clicks submit, they are taken to a results page that shows the prediction of Fresh or Rotten.

## Conclusion
We found that our model was able to accurately predict up to 71% accuracy. This shows that there is some level of predictability in tomatometer status, however still much to be analyzed. If we were to continue our research in the future, we would use the second dataset including the critics written reviews and use natural language processing to incorporate this feature in our model. We also discussed pulling another data set that might include budget or time spent making the film. Extra features like these might have given our model an advantage and may have increased model accuracy. We would also have included a dynamic dataset so that our model would remain current for future users. With these improvements, this model could prove to be an efficient algorithm to improve the movie makers ability to receive a Fresh review.
